...
namespace Battlehub.MeshDeformer2
{
    ...
    public class MeshDeformer : SplineBase
    {
        ...
        public void ResetDeformer()
        {
            ...
            {
                new ControlPointSetting(new Twist(0.0f, 0.0f, 1.0f), new Thickness(Vector3.one, 0.0f, 1.0f), new Wrap(0.0f, 0.0f, 1.0f)),
                new ControlPointSetting(new Twist(0.0f, 0.0f, 1.0f), new Thickness(Vector3.one, 0.0f, 1.0f), new Wrap(0.0f, 0.0f, 1.0f))
            };
        ...
        public void Split(int curveIndex)
        {
            if(CurveCount == 1)
            {
                throw new InvalidOperationException("CurveCount == 1");
            }

            if(curveIndex == 0 || curveIndex == CurveCount - 1)
            {
                throw new ArgumentOutOfRangeException("curveIndex", "[1, CurveCount - 2]");
            }

            for(int i = 0; i < CurveCount; ++i)
            {
                if(m_scaffolds[i].CurveIndices.Length > 1)
                {
                    throw new NotImplementedException("Split for MeshDeformer with multi-curve scaffolds is not implemented");
                }
            }

            GameObject copyGO = Instantiate(gameObject);
            MeshDeformer copyDeformer = copyGO.GetComponent<MeshDeformer>();
            for (int i = 0; i < curveIndex; ++i)
            {
                copyDeformer.RemoveCurve(0, false);
            }

            ScaffoldWrapper[] copyScaffolds = copyDeformer.m_scaffolds;
            for(int i = 1; i <= curveIndex; ++i)
            {
                Destroy(copyScaffolds[CurveCount - i].Obj.gameObject);
            }

            int copySize = CurveCount - curveIndex;
            Array.Resize(ref copyScaffolds, copySize);
            copyDeformer.m_scaffolds = copyScaffolds;

            for (int i = 0; i < copySize; ++i)
            {
                ScaffoldWrapper copyScaffold = copyScaffolds[i];
                copyScaffold.DuplicateMeshes();
                copyScaffold.Deform(copyDeformer, copyDeformer.Original, copyDeformer.ColliderOriginal);
            }
            
            
            int curveCount = CurveCount;
            for(int i = curveIndex; i < curveCount; ++i)
            {
                Remove(CurveCount - 1);
            }
        }
        ...
        private void ChangeLoop(bool loop)
        {
            if (m_spacing > 0)
            {
                ...
                else
                {
                    ...
                    if (m_spacing > 0)
                    {
                        PrependCurve(m_spacing, 0, true, false);
            ...
            if (m_spacing <= 0)
            {
                if (spacing > 0)
                {
                    ...
                    for (int s = scaffoldsList.Count - 1; s > to; s--)
                    {
                        ...
                        PrependCurve(points, curveIndex, mag, true, false);
                        offset--;
                    }
        ...
        {
            GameObject segment = CreateSegment();
            Vector3 from;
            Vector3 to;
            m_original.GetBounds(m_axis, out from, out to);
            ScaffoldWrapper previous = AppendSegment(segment, from, to);
        ...
        {
            ScaffoldWrapper previous = FindScaffold(CurveCount - 1);
            bool isRigid = false;
            if (previous != null)
            {
                isRigid = previous.IsRigid;
            ...
            ScaffoldWrapper scaffold = new ScaffoldWrapper(segment.AddComponent<Scaffold>(), isRigid);
        ...
        public GameObject Prepend()
        {
            ...
            Vector3 to;
            m_original.GetBounds(m_axis, out from, out to);
            ScaffoldWrapper previous = PrependSegment(curveIndex, segment, from, to, false);
        ...
        private ScaffoldWrapper PrependSegment(int curveIndex, GameObject segment, Vector3 from, Vector3 to, bool shrinkPreceding)
        {
            ...
            if (!Loop || shrinkPreceding)
            {
                ...
                if (m_spacing > 0 && !shrinkPreceding)
                {
                    PrependCurve(m_spacing, curveIndex, true, shrinkPreceding);
                ...
                PrependCurve(mag, curveIndex, false, shrinkPreceding);
                ...
                ScaffoldWrapper newScaffold = new ScaffoldWrapper(segment.AddComponent<Scaffold>(), isRigid);
            ...
            {
                previous = AppendSegment(segment, from, to);
            }
        ...
        public GameObject Insert(int curveIndex)
        {
            ...
            Vector3 to;
            m_original.GetBounds(m_axis, out from, out to);
            ScaffoldWrapper previous = PrependSegment(curveIndex, segment, from, to, true);
...
//        }
        ...
        private void ShiftAndInsert(int curveIndex, ScaffoldWrapper newScaffold)
        {
            ...
            if (scaffoldIndex == -1 && m_scaffolds.Length > 0 && m_scaffolds[m_scaffolds.Length - 1] == null)
            {
                //Insert command case
                scaffoldIndex = m_scaffolds.Length - 1;
            }
